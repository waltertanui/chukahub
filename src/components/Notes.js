...
const Notes = () => {
  ...
  const fetchData = async () => {
    if (selectedFaculty && selectedDepartment && selectedYear) {
      ...
      const q = query(
        uploadsRef,
        where('category', '==', 'notes'), // Filter by category
  ...
  <div className="flex items-center">
    ...
    <div className="logo py-2 px-4 ml-4 text-black text-lg font-bold">Chuka <span className="text-orange-500">Repository</span></div>
      ...
      <div className="uploads-container p-4 md:p-8">
        <h2 className="text-2xl mb-2 md:mb-4 font-serif text-center text-white">Notes</h2>
        <div className="p-4 md:p-30 bg-gray-900 justify-center rounded-md">
          <div className="flex flex-col md:flex-row justify-center items-center">
            <div className='p-4 md:p-16 bg-slate-400 w-full md:w-1/2 rounded-md'>
              ...
              <button onClick={handleGetNotes} className="bg-blue-500 text-white px-4 py-2 rounded-md w-full md:w-auto hover:bg-blue-600 transition duration-300">
                Get Notes
              </button>
            </div>
          </div>
          <div className="p-4 md:p-24 w-full md:w-1/2 mt-4 rounded-md flex flex-col md:flex-row gap-8">
            {uploads.length > 0 ? (
              ...
              <p className="mt-4 text-green-400">No notes found.</p>
            )}
          </div>
        </div>
      </div>
